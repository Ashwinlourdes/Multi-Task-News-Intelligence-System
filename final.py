...
def load_ml_classifier():
    tfidf = joblib.load("D:\\news\\models\\classification\\ml\\tfidf.pkl")
    clf = joblib.load("D:\\news\\models\\classification\\ml\\logreg.pkl")
    le = joblib.load("D:\\news\\models\\classification\\ml\\label_encoder.pkl")
    return tfidf, clf, le

@st.cache_resource
def load_bert_classifier():
    path = "D:\\news\\models1\\classification\\transformer\\bert_classifier"
...
@st.cache_resource
def load_rule_ner():
    with open("D:\\news\\ner1\\ml\\rule_patterns.json") as f:
        return json.load(f)

@st.cache_resource
def load_bert_ner():
    path = "D:\\news\\ner1\\transformer\\bert_ner"
    tokenizer = BertTokenizerFast.from_pretrained(path)
    model = BertForTokenClassification.from_pretrained(path)
    model.eval()
    id2tag = pickle.load(open("D:\\news\\ner1\\dl\\id2tag.pkl", "rb"))
...
def load_dl_classification():
    ...
    checkpoint_path = "D:\\news\\models1\\classification\\dl\\bilstm_classifier.pt"
    if os.path.exists(checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location="cpu")
        model.load_state_dict(checkpoint)
    model.eval()
    word2idx = pickle.load(open("D:\\news\\models1\\classification\\dl\\word2idx.pkl", "rb"))
    idx2label = pickle.load(open("D:\\news\\models1\\classification\\dl\\idx2label.pkl", "rb"))
...
@st.cache_resource
def load_bart():
    path = "D:\\news\\summarization1\\transformer\\bart_summarizer"
...
def load_seq2seq():
    ...
    seq2seq_dir = "D:\\news\\models1\\summarization\\dl"

    checkpoint_path = os.path.join(seq2seq_dir, "seq2seq_lstm.pt")
    ...
    # load vocab
    word2idx = pickle.load(open(os.path.join(seq2seq_dir, "word2idx.pkl"), "rb"))
    idx2word = pickle.load(open(os.path.join(seq2seq_dir, "idx2word.pkl"), "rb"))
...
def load_ml_summarizer():
    return pickle.load(open("D:\\news\\summarization1\\ml\\tfidf_extractive.pkl", "rb"))
...
if st.button("Run") and text.strip():
    ...
    if task == "Classification":
        if model_family == "From-Scratch ML":
            ...
            label = idx2label.get(pred, f"Class_{pred}") 
            st.success(f"Prediction: {label}")
